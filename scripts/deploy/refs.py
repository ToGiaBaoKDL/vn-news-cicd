...
def bundle_image_tag(repositories: dict[str, str]) -> str:
    payload = "\n".join(f"{name}={repositories[name]}" for name in sorted(repositories))
    return f"bundle-{hashlib.sha256(payload.encode()).hexdigest()[:16]}"
...
def resolve_deploy_refs(
    *,
    ...
    resolved_image_tag = image_tag or bundle_image_tag(repositories)
    validate_tag(resolved_image_tag, push=True)
    return resolved_image_tag, repositories
